      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          {team.map((member, index) => (
            <div
              key={index}
              className="rounded-xl bg-white/5 p-6 backdrop-blur-sm transition-all duration-300 hover:-translate-y-2 hover:transform hover:shadow-xl hover:shadow-[#0047AB]/20"
            >
              <div className="flex flex-col items-center">
                ...
                <div className="mb-4 flex flex-wrap justify-center gap-2">
                  {member.roles.map((role, roleIndex) => (
                    <span
                      key={roleIndex}
                      className="rounded-full bg-[#0047AB]/50 px-3 py-1 text-sm text-[#99c3ff]"
                    >
                      {role}
                    </span>
                  ))}
                </div>
                <p className="mb-4 text-center text-sm text-gray-400 sm:text-base pt-2 pb-2">
                  {member.intro}
                </p>
                <div className="flex space-x-4">
                  {Object.entries(member.urls).map(([key, url]) => (
                    <Link
                      ...
                      className="text-gray-400 transition-colors hover:text-[#0047AB]"
                    >
                      {key === "github" && (
                        <svg
                          className="h-5 w-5"
